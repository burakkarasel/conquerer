...
const createPostHandler = asyncWrapper(async (req, res) => {
...
const updatePostHandler = asyncWrapper(async (req, res) => {
  const { postId } = req.params;
  const { userId } = req;
  const { title, content } = req.body;

  if (title && title.length < 2) {
    throw generateCustomError("Title value must be at least 2 characters", 400);
  }
...
const getPostByIdHandler = asyncWrapper(async (req, res) => {
  const { postId } = req.params;
...
const listPostsHandler = asyncWrapper(async (req, res) => {
...
const deletePostHandler = asyncWrapper(async (req, res) => {
  const { postId } = req.params;
  const { userId } = req;
